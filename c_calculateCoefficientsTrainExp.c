...
void c_calculateCoefficientsTrainExp (double* x_logs, double* derivatives, double* x_log_eigenvals, double* coefficients, int n, int dd, int d) {
    ...
    for (i = 0; i < n; i++) {
        ...
        x_log_eigenval_i = x_log_eigenvals[i];                            
            
        for (j = 0; j < i; j++) {
                
            // log eigenvalue for this subject
            x_log_eigenval_j = x_log_eigenvals[j];
