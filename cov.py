...
import logging
from calculateCoefficientsCross import calculateCoefficientsCross
from calculateCoefficientsTrain import calculateCoefficientsTrain
from calculateCoefficientsCrossExp import calculateCoefficientsCrossExp
from calculateCoefficientsCrossExpPointer import calculateCoefficientsCrossExpPointer
from calculateCoefficientsTrainExp import calculateCoefficientsTrainExp
from calculateCoefficientsTrainExpPointer import calculateCoefficientsTrainExpPointer
...
class DiscLogECoeff(Kernel):
    ...
    '''
    Squared Exponential kernel with isotropic distance measure. hyp = [log_ell, log_sigma]
    Take a precomputed distance matrix (produced by arbitrary distance function/data)
    rather than raw data

    :param log_ell: characteristic length scale.
    :param log_sigma: signal deviation.
    :param log_a: coefficients of eigenvalues
    
    '''
    
    def __init__(self, D=0, log_ell=0., log_sigma=0., log_a_list=None):
        if log_a_list is None:
            # calculate matrix dimension d from data width D with quadratic formula
            d = -1 + np.sqrt(1 + 4 * D)/2           
            #self.hyp = [log_ell] + [log_sigma] + [0. for i in range(d)]
            self.hyp = [log_ell] + [log_sigma] + [0. for i in range(d)]
        else:
            #self.hyp = [log_ell] + [log_sigma] + log_a_list
            self.hyp = [log_ell] + [log_sigma] + log_a_list     
                       
    def calculateCoefficients(self, x_logs, derivatives, n):
        
        C = np.zeros((n, n))
        for i in range(n) :
            for j in range(i) :
                x_logs_diff = x_logs[i, :] - x_logs[j, :]      
                derivatives_diff = derivatives[i, :] - derivatives[j, :]
                C_element = np.sum(x_logs_diff * derivatives_diff)
                C[i, j] = C_element
                C[j, i] = C_element

        #C = np.sum((derivatives[:, np.newaxis, :] - derivatives[np.newaxis, :, :]) * (x_logs[:, np.newaxis, :] - x_logs[np.newaxis, :, :]), axis=2)
    ...
    def eigendecompositionsToLogs(self, x, a, d):
        
        # get number of subjects
        n = np.shape(x)[0]
               
        # allocate memory
        matrix_logs = np.zeros((n, d, d))
        
        # split into eigenvectors and eigenvalues
        eigenvectors = np.reshape(x[:, :d*d], (n, d, d))
        #eigenvectors = x[:, :d*d]
        eigenvalues = x[:, d*d:]
        
        # calculate logs from eigendecompostions
        for subject in range(n):
            
            V = eigenvectors[subject, :, :]
            D = np.diag(np.log(eigenvalues[subject, :] * a))
            matrix_logs[subject, :, :] = V.dot(D).dot(np.transpose(V))
            
        # reshape, squashing matrices to vectors to allow use of pdist/pdist2
        matrix_logs = np.reshape(matrix_logs, (n, d*d))        
        return matrix_logs
    ...
    def getCovMatrix(self,x=None,z=None,mode=None):
        ...
        if not z is None:
            nn, D = z.shape
        # get dimension d of matrices from x or z - use quadratic formula
        d = int((-1 + np.sqrt(1 + 4 * D))/2)
        ell = np.exp(self.hyp[0])    # characteristic length scale
        sf2 = np.exp(2.*self.hyp[1])      # signal variance
        a = np.exp(self.hyp[2:d+2])           # coefficients of Eigenvalues        
        if mode == 'self_test':           # self covariances for the test cases
            nn,D = z.shape
            A = np.zeros((nn,1))
        elif mode == 'train':             # compute covariance matix for dataset x
            x_logs = self.eigendecompositionsToLogs(x, a, d)
            x_dists = spdist.squareform(spdist.pdist(x_logs))
            A = (x_dists * x_dists) / (ell * ell)
        elif mode == 'cross':             # compute covariance between data sets x and z
            x_logs = self.eigendecompositionsToLogs(x, a, d)
            z_logs = self.eigendecompositionsToLogs(z, a, d)
            xz_dists = spdist.cdist(x_logs, z_logs)
            A = (xz_dists * xz_dists) / (ell * ell)
        A = sf2*np.exp(-0.5*A)
        return A
    ...
    def getDerMatrix(self,x=None,z=None,mode=None,der=None):
        ...
        if not z is None:
            nn, D = z.shape
        # get dimension d of matrices from x/z - use quadratic formula
        d = int((-1 + np.sqrt(1 + 4 * D))/2)
        ell = np.exp(self.hyp[0])  # characteristic length scale
        sf2 = np.exp(2.*self.hyp[1])        # signal variance
        a = np.exp(self.hyp[2:d+2])           # coefficients of Eigenvalues  
        if mode == 'self_test':           # self covariances for the test cases
            ...
            xz_dists = spdist.cdist(x_logs, z_logs)
            A = (xz_dists * xz_dists) / (ell * ell)
        if der == 0:                       # compute derivative matrix wrt length scale parameter
            A = sf2 * np.exp(-0.5*A) * A    
        elif der == 1:                    # compute derivative matrix wrt magnitude parameter
            A = 2 * sf2 * np.exp(-0.5*A)
        elif (der > 1 and der < d + 2): # compute derivative matrix wrt eigenvalue coefficient
            eigenvalue_ind = der - 2    # which eigenvalue are we taking the derivative wrt?
            if mode == 'self_test':
                A = A*0
            elif mode == 'train':
                ...
                # calculate coefficient array C
                # initialise it
                C = np.zeros((n, n))
                
                print 'eigenvalue ind = ' + str(eigenvalue_ind)
                
                # calculate derivatives of logm wrt the eigenvalue_indth eigenvalue
                # use this form of slicing due to C-style ordering of numpy.reshape
                # leading to eigenvectors not being in consecutive blocks
                start = eigenvalue_ind
                step = d
                stop = d*d
                kth_eigenvectors = x[:, start:stop:step]
                kth_eigenvectors_rep = np.repeat(kth_eigenvectors[:, :, np.newaxis], d, axis=2)
                logm_kth_eigenval_derivatives = np.reshape((kth_eigenvectors_rep * np.transpose(kth_eigenvectors_rep, (0, 2, 1))), (n, d*d))
                
                # calculate final derivatives with loop as it seems fastest??????
                for i in range(n) :
                    for j in range(i) :
                        x_logs_diff = x_logs[i, :] - x_logs[j, :]      
                        logm_kth_eigenval_derivatives_diff = logm_kth_eigenval_derivatives[i, :] - logm_kth_eigenval_derivatives[j, :]
                        C_element = np.sum(x_logs_diff * logm_kth_eigenval_derivatives_diff)
                        C[i, j] = C_element
                        C[j, i] = C_element
                         
                         
                # calculate final derivatives with broadcasting
                #C = np.sum((logm_kth_eigenval_derivatives[:, np.newaxis, :] - logm_kth_eigenval_derivatives[np.newaxis, :, :]) * (x_logs[:, np.newaxis, :] - x_logs[np.newaxis, :, :]), axis=2)
                
                # calculate final coefficients with Cython extension
                #calculateCoefficientsTrain(x_logs, logm_kth_eigenval_derivatives, C)
                
                A = sf2 * np.exp(-0.5*A) * C * (1 / (ell * ell))
                ...
                step = d
                stop = d*d
                kth_eigenvectors_x = x[:, start:stop:step]
                kth_eigenvectors_x_rep = np.repeat(kth_eigenvectors_x[:, :, np.newaxis], d, axis=2)
                logm_kth_eigenval_derivatives_x = np.reshape((kth_eigenvectors_x_rep * np.transpose(kth_eigenvectors_x_rep, (0, 2, 1))), (n, d*d))
                kth_eigenvectors_z = z[:, start:stop:step]
                kth_eigenvectors_z_rep = np.repeat(kth_eigenvectors_z[:, :, np.newaxis], d, axis=2)
                logm_kth_eigenval_derivatives_z = np.reshape((kth_eigenvectors_z_rep * np.transpose(kth_eigenvectors_z_rep, (0, 2, 1))), (nn, d*d))
...
#                        C[j, i] = C_element
                ...
                # calculate final coefficients with Cython extension
                calculateCoefficientsCross(x_logs, logm_kth_eigenval_derivatives_x, z_logs, logm_kth_eigenval_derivatives_z, C)
                         
                A = sf2 * np.exp(-0.5*A) * C * (1 / (ell * ell))
         
        else:
            raise Exception("Wrong hyperparamater index in DiscLogECoeff")
        return A
    
class DiscLogEExp(Kernel):
    ...
    :param log_ell: characteristic length scale.
    :param log_sigma: signal deviation.
    :param a: exponents of eigenvalues
    
    '''
    
    def __init__(self, D=0, log_ell=0., log_sigma=0., a_list=None):
        if a_list is None:
            # calculate matrix dimension d from data width D with quadratic formula
            d = -1 + np.sqrt(1 + 4 * D)/2           
            #self.hyp = [log_ell] + [log_sigma] + [0. for i in range(d)]
            ...
            #self.hyp = [log_ell] + [log_sigma] + log_a_list
            self.hyp = [log_ell] + [log_sigma] + a_list     
    ...
    def eigendecompositionsToLogs(self, x, a, d):
        ...
        for subject in range(n):
            ...
            V = eigenvectors[subject, :, :]
            #D = np.diag(np.log(eigenvalues[subject, :] * a))
            D = np.diag(np.log(np.power(eigenvalues[subject, :], a)))
            matrix_logs[subject, :, :] = V.dot(D).dot(np.transpose(V))
    ...
    def getCovMatrix(self,x=None,z=None,mode=None):
        ...
        ell = np.exp(self.hyp[0])    # characteristic length scale
        sf2 = np.exp(2.*self.hyp[1])      # signal variance
        a = self.hyp[2:d+2]         # powers of Eigenvalues        
        if mode == 'self_test':           # self covariances for the test cases
            nn,D = z.shape
            A = np.zeros((nn,1))
        elif mode == 'train':             # compute covariance matix for dataset x
    ...
    def getDerMatrix(self,x=None,z=None,mode=None,der=None):
        ...
        ell = np.exp(self.hyp[0])  # characteristic length scale
        sf2 = np.exp(2.*self.hyp[1])        # signal variance
        a = self.hyp[2:d+2]           # coefficients of Eigenvalues  
        if mode == 'self_test':           # self covariances for the test cases
            nn,D = z.shape
            A = np.zeros((nn,1))
        elif mode == 'train':             # compute covariance matix for dataset x
        ...
        if der == 0:                       # compute derivative matrix wrt length scale parameter
            ...
            if mode == 'self_test':
                ...
                # calculate logs of original eigenvalues
                log_eigenvalues = np.log(np.sqrt(x[:, d*d:]))               
...
#                    for j in range(i) :
#                        x_logs_diff = x_logs[i, :] - x_logs[j, :]      
#                        logm_kth_eigenval_derivatives_diff = (logm_kth_eigenval_derivatives[i, :] * log_eigenvalues[i, eigenvalue_ind]) - (logm_kth_eigenval_derivatives[j, :] * log_eigenvalues[j, eigenvalue_ind])
#                        C_element = np.sum(x_logs_diff * logm_kth_eigenval_derivatives_diff)
#                        C[i, j] = C_element
#                        C[j, i] = C_element
                ...
                # calculate final coefficients with Cython extension
                #kth_log_eigenvalues = np.copy(log_eigenvalues[:, eigenvalue_ind])
                #calculateCoefficientsTrainExp(x_logs, logm_kth_eigenval_derivatives, kth_log_eigenvalues, C)
                calculateCoefficientsTrainExpPointer(x_logs, logm_kth_eigenval_derivatives, log_eigenvalues[:, eigenvalue_ind], C)
                
                # call a function to calculate C
                #C = self.calculateCoefficientsTrainFoo(x_logs, logm_kth_eigenval_derivatives, log_eigenvalues[:, eigenvalue_ind], n)
                ...
                # calculate logs of original eigenvalues
                log_eigenvalues_x = np.log(np.sqrt(x[:, d*d:]))
                log_eigenvalues_z = np.log(np.sqrt(x[:, d*d:]))
                
                
                # calculate final derivatives with loop as it seems fastest??????
#                for i in range(n) :
#                    for j in range(i) :
#                        xz_logs_diff = x_logs[i, :] - z_logs[j, :]      
#                        logm_kth_eigenval_derivatives_diff = (logm_kth_eigenval_derivatives_x[i, :] * log_eigenvalues_x[i, eigenvalue_ind]) - (logm_kth_eigenval_derivatives_z[j, :] * log_eigenvalues_z[j, eigenvalue_ind])
#                        C_element = np.sum(xz_logs_diff * logm_kth_eigenval_derivatives_diff)
#                        C[i, j] = C_element
#                        C[j, i] = C_element

                # calculate final coefficients with Cython extension
                calculateCoefficientsCrossExpPointer(x_logs, logm_kth_eigenval_derivatives_x, log_eigenvalues_x[:, eigenvalue_ind], z_logs, logm_kth_eigenval_derivatives_z, log_eigenvalues_z[:, eigenvalue_ind], C)
